...
public class Controller {
    ...
    public void queryAllSpiceName(){
        XQResultSequence xqrs = executeQuery("for $SPICE in doc('/db/spice/spices.xml')/SPICES/SPICE return $SPICE/SPICE_NAME");
    ...
    public void queryByCountry(String country){
        XQResultSequence xqrs = executeQuery( "for $SPICE in doc('/db/spice/spices.xml')/SPICES/SPICE where contains ($SPICE/SPICE_COUNTRY_ORIGIN,'" + country +"')" + " return $SPICE/SPICE_NAME");
        printResultSequence(xqrs);
    }
    /**
    ...
    public void queryByCuisine(String cuisine){
        XQResultSequence xqrs = executeQuery( "for $SPICE in doc('/db/spice/spices.xml')/SPICES/SPICE where contains ($SPICE/SPICE_CUISINE,'" + cuisine +"')" + " return $SPICE/SPICE_NAME");
        printResultSequence(xqrs);
    }
    public void queryByFormat(String format){
        XQResultSequence xqrs = executeQuery( "for $SPICE in doc('/db/spice/spices.xml')/SPICES/SPICE where contains ($SPICE/SPICE_PRODUCT_STYLE,'" + format +"')" + " return $SPICE/SPICE_NAME");
        printResultSequence(xqrs);
    }
    /**
    ...
    public void insertSpice(String name, String description, String format, String country){
        try {
            ...
            String xquery = "update insert \n" +
                    ...
                    "</SPICE>" + " into doc('/db/spice/spices.xml')/SPICES" ;
    ...
    public void deleteSpiceByName(String name){
        try {
            xqe = conn.createExpression();
            String xquery = "update delete doc('/db/spice/spices.xml')" +
    ...
    public void updateSpiceByName(String name, String newName) {
        try {
            xqe = conn.createExpression();
            String xquery = "update value doc('/db/spice/spices.xml')" +
    ...
    public void deleteSpiceByCountry(String country) {
        try {
            xqe = conn.createExpression();
            String xquery = "update delete doc('/db/spice/spices.xml')/SPICES/SPICE[SPICE_COUNTRY_ORIGIN='" + country + "']";
            System.out.println(xquery);
            xqe.executeCommand(xquery);
        } catch (XQException e) {
            e.printStackTrace();
    ...
    public void deleteSpiceByProductStyle(String productStyle) {
        try {
            xqe = conn.createExpression();
            String xquery = "update delete doc('/db/spice/spices.xml')/SPICES/SPICE[SPICE_PRODUCT_STYLE='" + productStyle + "']";
            System.out.println(xquery);
            xqe.executeCommand(xquery);
        } catch (XQException e) {
            e.printStackTrace();
